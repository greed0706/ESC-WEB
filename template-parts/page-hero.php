...
// variant 'banner' = chỉ hiện trọn ảnh banner (tỉ lệ gốc). Mặc định tiêu đề đã
// bake sẵn trong ảnh nên KHÔNG render h1 overlay (chỉ đặt aria-label cho
// section). banner_title=true (vd /ve-ecs/) là NGOẠI LỆ: ảnh banner KHÔNG có
// chữ bake sẵn (khác các trang banner khác), Figma đặt tiêu đề là layer text
// thật đè lên ảnh -> render h1 overlay theo đúng toạ độ layer đó.
...
$banner_title   = ! empty( $args['banner_title'] );
...
if ( $is_banner ) {
	...
	if ( $banner_title ) {
		$hero_class .= ' ecs-page-hero--banner-title';
	}
}
?>
<?php if ( $is_banner ) : ?>
